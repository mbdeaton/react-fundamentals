...
#### Key takeaway
Build up a webpage from nodes in React using
1. `React.createElement(thisTag, theseProps)`
2. `ReactDOM.createRoot(thisRoot).render(thisElement)`

Note, the `React.createElement` footprint with children can take many forms.
Each of the following is equivalent:
```
React.createElement(thisTag, {children: [child1, child2]})
React.createElement(thisTag, null, [child1, child2])
React.createElement(thisTag, null, child1, child2)
```

#### Tips
React children can be object literals, like strings.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=brett.deaton%40gmail.com).
