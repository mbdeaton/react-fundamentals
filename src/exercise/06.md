...
You have access to named input values from a form with

```jsx
function AlertInput() {
  return (
    <form onSubmit={() => alert(event.target.elements.uname.value)}>
      <input type="text" id="uname" />
    </form>
  )
}
const element = <AlertInput />
```

Instead of digging through the event object, you could use the React-specific
state container useRef():
...
function AlertInput() {
  ...
  return (
    <form onSubmit={() => alert(inputRef.current.value)}>
      <input type="text" id="uname" ref={inputRef} />
    </form>
  )
}
const element = <AlertInput />
...
And another React hook, useState is useful to share state between components
(?):

```jsx
function AlertInput() {
  const inputRef = React.useRef()
  const [inputState, setInputState] = React.useState('initial value')
  function handleChange(event) {
    setInputState('changed value')
  }
  return (
    <form onSubmit={() => alert(inputRef.current.value)}>
      <input type="text" id="uname" ref={inputRef} onChange={handleChange} />
      <p>{inputState}</p>
    </form>
  )
}
...
Or I could control the input programmatically using the input property
`value`:
...
function AlertInput() {
  ...
  function handleChange(event) {
    setMyInput(doSomethingToTheInput(event.target.value))
  }
  return (
    <form onSubmit={() => alert(myInput)}>
      <input type="text" id="uname" value={myInput} onChange={handleChange} />
    </form>
  )
}
const element = <AlertInput />
...
Turn off the default refresh for a form with `event.preventDefault()`.

Recall the ES6 destructuring syntax to unpack an array
`let [x, y] = funcReturningAnArray`. This is similar to the Python syntax:
`x, y = func_returning_a_list_or_tuple`.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=brett.deaton%40gmail.com).
