...
#### Key takeaways
JSX simplifies the `createElement` calls:

```
React.createElement(thisTag, theseProps, theseChildren)
```

is equivalent to

```
<thisTag {...theseProps}>{theseChildren}</thisTag>
```

To use JSX, include a precompiler, e.g. babel with
`<script src=...babel...></script>`,
and change your source script `type` attribute from `"module"`
to `"text/babel"`.

#### Tips
Format a block of code in VSCode with the shortcut ctrl+shift+I.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=brett.deaton%40gmail.com).
