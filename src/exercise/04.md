...
One way to organize code is to create functions that return React components.
But this isn't the canonical way, it's just an intro to get thinking about
functions returning React components.

#### Notes

Since this exercise is so fundamental, here are the logical steps Dodds guides
us through to understand creating custom components:

```jsx
element = <div className="message">'Hello World'</div>
```

to

```jsx
function message({children}) {                       // <--create function
  return <div className="message">{children}</div>
}
element = {message({children: 'Hello World})}        // <--call function
```
...
element = {React.createElement('message', {className: 'message', children: 'Hello World})} // <--call createElement
```
...
```jsx
function Message({children}) {
  // <--capitalize for babel
  return <div className="message">{children}</div>
}
element = <Message>Hello World</Message> // <--use JSX
```
...
const PropTypes = {                   // <--introduce validation functions
  string(props, propName, componentName) {
    if (typeof props[propName] !== 'string') {
      return new Error(`Component must be a string`) }}
}
Message.propTypes = {
  children: PropTypes.string,         // <--bind to component
}
...
In a function definition, if you're using a small number of properties of an
object argument, like this:
...
you can destructure that argument like this:
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=brett.deaton%40gmail.com).
