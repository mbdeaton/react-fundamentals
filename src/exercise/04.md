...
#### Key takeaway
One way to organize code is to create functions that return
React components. But this isn't the canonical way, it's just an intro to
get thinking about functions returning React components.

#### Notes
Since this exercise is so fundamental, here are the logical steps Dodds
guides us through to understand creating custom components:
...
to
```jsx
function Message({children}) {                   // <--capitalize for babel
  return <div className="message">{children}</div>
}
