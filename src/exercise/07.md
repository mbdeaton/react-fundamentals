...
#### Key takeaway
Add a key to any array elements, so React can track their state. That
needs to be a unique identifier for that item, not the array index,
which as the array changes, will give the same key to different components.

This is true for all React components with state. But it's often visible with
arrays because they're commonly used to group copies of components together.

#### Tips
It's often useful to disable a button based on some state.
Use the `disabled` prop, like:

```jsx
<button disabled={Boolean(items)} onClick={handleClick}>Add Items<button/>
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=07%3A%20Rendering%20Arrays&em=brett.deaton%40gmail.com).
