...
Inline styles are applied in JSX with the style prop:

```jsx
<div style={{backgroundColor: '#aaa'}}>blah</div>
```

Whereas styles sourced from an external stylesheet are applied using the
className prop:
...
In JSX we use DOM properties (e.g. `className`), not HTML attributes (e.g.
`class`). Note, in the browser devtools the selected DOM node is bound to the
handle `$0`. So you can access the DOM properties of the selected element in the
console with `$0.blahProperty`. To see the styles applied to the element with
their DOM property names do `$0.style`.

In ES6 you can merge props `blahProp` and objects `xObj` with the spread
operator: `{blahProp: 'blahVal', ...x}`.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=brett.deaton%40gmail.com).
